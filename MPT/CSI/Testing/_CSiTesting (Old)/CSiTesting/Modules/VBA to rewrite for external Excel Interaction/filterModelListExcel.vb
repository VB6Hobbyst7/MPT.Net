...
Module filterModelListExcel
    ...
    '    colMax = columnNumberToLetter(Range("list_filter_id").Column)    'Longest column, or if unknown, starting column
    ...
    '    'Clears columns
    '    clearColumn("keyword_multi_any")
    'End Sub

    'Sub clearModelXMLbyFilter_List()
    '    'Clears columns
    '    clearColumn("list_filter_id_secondary")
    '    clearColumn("list_filter_title")
    '    clearColumn("list_filter_program")
    '    clearColumn("list_filter_run_time")
    '    clearColumn("list_filter_id")
    'End Sub
    ...
    '    'Clears Columns
    '    clearColumn("dropdownList_keywordsHeader", , , , "Dropdown Lists", -1)

    '    path = regTest.pathModels

    '    'Sets up region boundaries
    '    colMax = columnNumberToLetter(Range("dropdownList_keywordsHeader").Column)    'Longest column, or if unknown, starting column
    '    rowGlobal = Worksheets(SName).Range(colMax & "65536").End(xlUp).row 'Letter should be of column with greatest number of filled rows
    ...
    '            rowGlobal = Worksheets(SName).Range(colMax & "65536").End(xlUp).row + 1     ' next row number. Letter should be of column with greatest number of filled rows
    '        End If
    '    Next FileItem
    ...
    '            If selectModel Then rowGlobal = Range(colMax & "65536").End(xlUp).row + 1 ' next row number. Letter should be of column with greatest number of filled rows
    '        End If
    '    Next FileItem
    ...
    '    'Method
    '    While Not cellBlank
    '        cellText = Range(keywordCell).Offset(i + 1, 0).value
    ...
    '        cellText = Range(namedRange).Offset(i, 0).value
    '        If cellText = "" Then
    '            cellBlank = True
    '        Else
    ...
    '    fillAll("yes", "list_filter_add", "list_filter_id")
    'End Sub

    'Sub selectNone()
    '    'Deselects all listed models
    '    fillAll("", "list_filter_add", "list_filter_id")
    'End Sub
    ...
    '    If Range(booleanRange).value = 2 Then fillAll("", fillRange, referenceRange) 'Clears selection before adding new one

    '    'Selects corresponding cells
    '    For Each cell In Selection
    '        rowOffset = cell.row - Range(fillRange).row
    '        Range(fillRange).Offset(rowOffset, 0).value = "yes"
    '    Next cell
    'End Sub
    ...
    '    clearColumn("model_id")

    '    'Create array of rows to use
    '    While Not cellBlank
    '        cellText = Range("list_filter_id").Offset(i, 0).value
    '        If cellText = "" Then ' Stops checking cells at the end of the models list
    '            cellBlank = True
    '        Else
    '            If Range("list_filter_add").Offset(i, 0).value = "yes" Then
    '                ReDim Preserve modelsSelected(j)
    '                modelsSelected(j) = Range("list_filter_id").Offset(i, 0).value
    '                j = j + 1
    '            End If
    '            i = i + 1
    '        End If
    ...
    '    For j = 0 To UBound(modelsSelected)
    '        Range("model_id").Offset(j + 1, 0).value = modelsSelected(j)
    '    Next j
