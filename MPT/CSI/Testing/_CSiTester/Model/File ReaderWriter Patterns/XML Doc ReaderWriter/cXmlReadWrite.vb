...
#Region "Properties"
    Friend Property xmlDoc As New XmlDocument
    Friend Property xmlRoot As XmlElement
    Friend Property myXMLNode As XmlNode
    Friend Property myXMLObject As XmlNode
    Friend Property myXMLAttribute As XmlAttribute
...
#Region "XML Read/Write Master Functions"
    ...
    ''' <remarks></remarks>
    Friend Function InitializeXML(ByVal p_path As String,
    ...
    ''' <remarks></remarks>
    Friend Sub SaveXML(ByVal p_path As String,
                              Optional ByVal p_noOverWrite As Boolean = False)
        Try
            If FileExists(p_path) Then
                If Not p_noOverWrite Then SetFileNotReadOnly(p_path) 'Sets the specified file to being readable if it is 'read only'.

                xmlDoc.Save(p_path) 'Saves file if XML is modified
            End If
        Catch ex As Exception
            csiLogger.ExceptionAction(ex)
        End Try
    ...
    ''' <remarks></remarks>
    Friend Sub ClearObjects(ByVal p_pathNode As String,
...
#Region "Get XML/Node Information"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function ChildCount(ByVal myPath As String) As Integer
        Try
            'Create an XmlNamespaceManager for resolving namespaces.
            Dim nsmgr As New XmlNamespaceManager(xmlDoc.NameTable)
            nsmgr.AddNamespace("n", xmlDoc.DocumentElement.NamespaceURI)
    ...
    ''' <remarks></remarks>
    Friend Function CountChildNodes(ByVal pathNode As String) As Integer
        Dim i As Integer
    ...
    ''' <remarks></remarks>
    Friend Function XMLChildCount(ByVal myNode As XmlNode) As Integer
        'Counts children based on 3 cases
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function NodeExists(ByVal pathNode As String) As Boolean
        Try
            'Create an XmlNamespaceManager for resolving namespaces.
            Dim nsmgr As New XmlNamespaceManager(xmlDoc.NameTable)
            nsmgr.AddNamespace("n", xmlDoc.DocumentElement.NamespaceURI)

            If xmlRoot.SelectSingleNode(pathNode, nsmgr) Is Nothing Then
                Return False
            Else
                Return True
            End If
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function AttributeExists(ByVal node As XmlNode, ByVal pathNodeAttrib As String) As Boolean
        If node.NodeType = XmlNodeType.Comment Then Return False
        If node.Attributes.GetNamedItem(pathNodeAttrib) Is Nothing Then
            Return False
        Else
            Return True
        End If
...
#Region "Reading/Writing Nodes"
    ...
    ''' <remarks></remarks>
    Friend Function ReadNodeText(ByVal pathNode As String, Optional ByVal pathNodeAttrib As String = "") As String
        Try
            'Create an XmlNamespaceManager for resolving namespaces.
            Dim nsmgr As New XmlNamespaceManager(xmlDoc.NameTable)
            nsmgr.AddNamespace("n", xmlDoc.DocumentElement.NamespaceURI)
            ...
            If NodeExists(pathNode) Then
                ...
                If String.IsNullOrEmpty(pathNodeAttrib) Then
    ...
    ''' <remarks></remarks>
    Friend Sub WriteNodeText(ByVal p_propValue As String,
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function ReadChildText(ByVal pathNode As String, ByVal childNum As Integer, ByVal childChildNum As Integer) As String
        Try
            'Create an XmlNamespaceManager for resolving namespaces.
            Dim nsmgr As New XmlNamespaceManager(xmlDoc.NameTable)
            nsmgr.AddNamespace("n", xmlDoc.DocumentElement.NamespaceURI)
            ...
            If myXMLNode.ChildNodes.Item(childNum).ChildNodes.Item(childChildNum) IsNot Nothing Then
    ...
    ''' <remarks></remarks>
    Friend Sub GetSingleXMLNodeValue(ByVal myPath As String, ByVal myPathNode As String, ByRef myNodeValue As String, Optional ByVal myPathNodeAttrib As String = "", Optional ByVal cleanPath As Boolean = False)
    ...
    ''' <param name="cleanPath">If true, the path will be trimmed of white space, leading and ending "\", and all "\\".</param>
    ''' <remarks></remarks>
    Friend Function GetSingleXMLNodeListValues(ByVal pPath As String, ByVal pPathNode As String, Optional ByVal pPathNodeAttrib As String = "", Optional ByVal cleanPath As Boolean = False) As ObservableCollection(Of String)
...
#Region "Reading/Writing Node Lists"
    ...
    ''' <returns>Node value or node attribute value.</returns>
    ''' <remarks></remarks>
    Friend Function ReadNodeListText(ByVal pathNode As String, ByRef myList As ObservableCollection(Of String), Optional ByVal pathNodeAttrib As String = "") As Boolean
        Dim i As Integer
        ...
        Try
            ...
            'Lookup node or attribute within XML file
            If myXMLNode IsNot Nothing Then
                If String.IsNullOrEmpty(pathNodeAttrib) Then                 'Get node text
                    For i = 0 To myXMLNode.ChildNodes.Count - 1
    ...
    ''' <remarks></remarks>
    Friend Sub WriteNodeListText(ByVal propValue As String(), ByVal pathNode As String, ByVal nameListNode As String)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function ReadNodeListPath(ByVal pathNode As String, Optional ByVal myRelativeToProgram As String = "") As ObservableCollection(Of String)
    ...
    ''' <param name="myRelativeToProgram">If base reference is relative to the program, specify the relative path difference with this parameter.</param>
    ''' <remarks></remarks>
    Friend Sub WriteNodeListPath(ByVal pathNode As String, ByVal nameListNode As String, ByVal pathList As ObservableCollection(Of String), Optional ByVal myRelativeToProgram As String = "")
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function ReadUniqueList(ByVal pathNode As String) As ObservableCollection(Of String)
        Dim myList As New ObservableCollection(Of String)
    ...
    ''' <remarks></remarks>
    Friend Sub WriteUniqueList(ByVal pathNode As String, ByVal nameListNode As String, ByVal writeList As ObservableCollection(Of String))
        Dim tempList As New List(Of String)
    ...
    ''' <remarks></remarks>
    Friend Sub WriteSingleXMLNodeValue(ByVal p_path As String,
                                              ByVal p_pathNode As String,
...
#Region "Reading/Writing Node Objects"
    ...
    ''' <param name="p_list">Optional: List to be populated by the text values of multiple child elements.</param>
    ''' <param name="p_attributeName">Optional: Attribute to populate to the list, if supplied.</param>
    ''' <param name="p_useNameSpace">Optional: If true, a namespace element is used in the paths and must be supplied with the input paths.</param>
    ''' <param name="p_parentNodes">Optional: Name of parent node for truncating the scope of where the list of nodes is gathered.</param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function ReadXmlObjectText(ByVal p_tagName As String,
                                      ...
                                      Optional ByVal p_parentNodes As List(Of String) = Nothing) As String
        Dim nodeValue As String
        Dim nodelist As XmlNodeList = Nothing

        p_list.Clear()  'Ensures that list starts out empty
        ReadXmlObjectText = ""
        ...
        Try
            ...
            If Not GetNodeList(nodelist, p_tagName, p_useNameSpace, xmlDoc.DocumentElement.NamespaceURI, p_parentNodes) Then Exit Function
            ...
            For Each node As XmlNode In nodelist
                ...
                If Not String.IsNullOrEmpty(nodeValue) Then p_list.Add(nodeValue)
            Next
        Catch ex As Exception
            csiLogger.ExceptionAction(ex)
            CloseXML()
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function GetResultsNodeList(ByVal p_tagName As String,
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function GetNodeList(ByRef nodeList As XmlNodeList, ByVal tagName As String, ByVal useNameSpace As Boolean, Optional ByVal nameSpaceURI As String = "", Optional ByVal parentNodes As List(Of String) = Nothing) As Boolean
        Try
            If useNameSpace Then
                If ((parentNodes Is Nothing OrElse
                     parentNodes.Count = 0) AndAlso
                    xmlDoc.GetElementsByTagName(tagName, nameSpaceURI) IsNot Nothing) Then

                    nodeList = xmlDoc.GetElementsByTagName(tagName, nameSpaceURI)
                    Return True
                Else
                    If (xmlDoc.Item(parentNodes(0)) IsNot Nothing AndAlso
                        xmlDoc.Item(parentNodes(0)).Item(parentNodes(1)) IsNot Nothing AndAlso
                        xmlDoc.Item(parentNodes(0)).Item(parentNodes(1)).GetElementsByTagName(tagName) IsNot Nothing) Then
            ...
            Else
                If ((parentNodes Is Nothing OrElse
                     parentNodes.Count = 0) AndAlso
                     xmlDoc.GetElementsByTagName(tagName) IsNot Nothing) Then

                    nodeList = xmlDoc.GetElementsByTagName(tagName)
                    Return True
                ElseIf (xmlDoc.Item(parentNodes(0)) IsNot Nothing AndAlso
                        xmlDoc.Item(parentNodes(0)).Item(parentNodes(1)) IsNot Nothing AndAlso
                        xmlDoc.Item(parentNodes(0)).Item(parentNodes(1)).GetElementsByTagName(tagName) IsNot Nothing) Then
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function SelectSingleNode(ByVal node As XmlNode, ByVal singleNodePath As String, ByVal useNameSpace As Boolean, Optional nsmgr As XmlNamespaceManager = Nothing, Optional ByVal attributeName As String = "") As String
        Dim nodeValue As String = ""

        Try
            If useNameSpace Then
                If node.SelectSingleNode(singleNodePath, nsmgr) IsNot Nothing Then
                    If (Not String.IsNullOrEmpty(attributeName) AndAlso
                        node.SelectSingleNode(singleNodePath, nsmgr).Attributes.GetNamedItem(attributeName) IsNot Nothing) Then

                        nodeValue = node.SelectSingleNode(singleNodePath, nsmgr).Attributes.GetNamedItem(attributeName).InnerText
                    ElseIf (Not String.IsNullOrEmpty(attributeName) AndAlso
                            node.SelectSingleNode(singleNodePath, nsmgr).Attributes.GetNamedItem(attributeName) Is Nothing) Then
                        nodeValue = ""
                    Else                                    'Get node value
                        nodeValue = node.SelectSingleNode(singleNodePath, nsmgr).InnerText
                    End If
                End If
            ElseIf node.SelectSingleNode(singleNodePath) IsNot Nothing Then
                If (Not String.IsNullOrEmpty(attributeName) AndAlso
                      node.SelectSingleNode(singleNodePath).Attributes.GetNamedItem(attributeName) IsNot Nothing) Then
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function ConfirmationNodeExists(ByVal confirmationNode As String, ByVal useNameSpace As Boolean, ByVal nsmgr As XmlNamespaceManager, ByVal nodeList As XmlNodeList) As Boolean
        Try
            If Not String.IsNullOrEmpty(confirmationNode) Then
                If useNameSpace Then
                    For Each node As XmlNode In nodeList
                        If node.SelectSingleNode(confirmationNode, nsmgr) IsNot Nothing Then Return True
                    Next
                Else
                    For Each node As XmlElement In nodeList
                        If node.SelectSingleNode(confirmationNode) IsNot Nothing Then Return True
                    Next
                End If
                Return False
            End If
    ...
    ''' <remarks></remarks>
    Friend Sub UpdateObjectByTag(ByVal tagName As String, ByVal lookupNodePath As String, ByVal writeNodePath As String, ByVal nodeValueLookup As String, ByVal nodeValueWrite As String, _
        ...
        Try
            ...
            If useNameSpace Then
                If xmlDoc.GetElementsByTagName(tagName, xmlDoc.DocumentElement.NamespaceURI) IsNot Nothing Then
                    nodelist = xmlDoc.GetElementsByTagName(tagName, xmlDoc.DocumentElement.NamespaceURI)
                Else
                    Exit Sub
                End If
            Else
                If xmlDoc.GetElementsByTagName(tagName) IsNot Nothing Then
                    nodelist = xmlDoc.GetElementsByTagName(tagName)
                Else
                    Exit Sub
                End If
            ...
            If useNameSpace Then
                For Each node As XmlNode In nodelist
                    If node.SelectSingleNode(lookupNodePath, nsmgr) IsNot Nothing Then
                        If Not String.IsNullOrEmpty(attributeName) Then  'Get attribute value
                            If node.SelectSingleNode(lookupNodePath, nsmgr).Attributes.GetNamedItem(attributeName) IsNot Nothing Then
            ...
            Else
                For Each node As XmlElement In nodelist
                    If node.SelectSingleNode(lookupNodePath) IsNot Nothing Then
                        If Not String.IsNullOrEmpty(attributeName) Then  'Get attribute value
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function ReadXmlObjectTextSubTag(ByVal tagName As String, ByVal tagIdentifier As String, ByVal tagIdentifierValue As String, ByVal singleNodePath As String, ByRef myList As ObservableCollection(Of String), Optional ByVal singleNodePathParent As String = "", _
        ...
        Try
            ...
            For Each node As XmlNode In nodeList
                If node.SelectSingleNode(tagIdentifier, nsmgr) IsNot Nothing Then
                    If node.SelectSingleNode(tagIdentifier, nsmgr).InnerText = tagIdentifierValue Then
                        If Not String.IsNullOrEmpty(singleNodePathParent) Then
                            If useNameSpace Then
                                If node.SelectSingleNode(singleNodePathParent, nsmgr) IsNot Nothing Then
                                    nodeSubList = node.SelectSingleNode(singleNodePathParent, nsmgr).ChildNodes
                                End If
                            Else
                                If node.SelectSingleNode(singleNodePathParent) IsNot Nothing Then
                        ...
                        If nodeSubList IsNot Nothing Then
                            For Each subNode As XmlNode In nodeSubList
                                nodeValue = ""
                                If String.IsNullOrEmpty(singleNodePath) Then
                                    If Not subNode.NodeType = XmlNodeType.Comment Then
                                        If String.IsNullOrEmpty(singleNodePathChildren) Then
                                            nodeValue = subNode.InnerText
                                        Else
                                            If useNameSpace Then
                                                If subNode.SelectSingleNode(singleNodePathChildren, nsmgr) IsNot Nothing Then
                                                    nodeValue = subNode.SelectSingleNode(singleNodePathChildren, nsmgr).InnerText
                                                End If
                                            Else
                                                If subNode.Item(singleNodePathChildren) IsNot Nothing Then
                                ...
                                End If
                                If Not String.IsNullOrEmpty(nodeValue) Then myList.Add(nodeValue)
                            Next
                        End If
                    End If
                End If
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function ReadXmlObjectTextSubSubTag(ByVal tagName As String, ByVal subTagName As String, ByVal subSubTagName As String, ByVal singleNodePath As String, ByRef myList As ObservableCollection(Of String)) As String
        Dim nodeValue As String
    ...
    ''' <remarks></remarks>
    Friend Function ListObjectByKey(ByVal pathNode As String, ByVal keyNodeValue As String, ByVal lookupNodeName As String) As String
    ...
    ''' <remarks></remarks>
    Friend Sub ReadXmlSettingsObject(ByVal pathNode As String, ByVal myObjectType As eXMLSettingsObjectType)
...
#Region "Reading/Writing Specific Cases"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function UpdateUniqueModelIDList(Optional ByVal resultsPath As String = "") As List(Of String)
        Dim tempList As New List(Of String)

        'Open XML File
        If Not String.IsNullOrEmpty(resultsPath) Then
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function UpdateModelIDListComponent() As List(Of String)
        Dim j As Integer
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function UpdateUniqueModelIDListComponent() As List(Of String)
        Dim j As Integer
        Dim pathNode As String = "//n:items"
        Dim tempList As New List(Of String)
        Dim tempEntry As String
    ...
    ''' <remarks></remarks>
    Friend Sub HandleZeroTolerance(ByVal nodeIndex As Integer, ByVal resultType As eResultType, Optional ByVal attributeValue As String = "1", Optional ByVal overWriteExisting As Boolean = True)
    ...
    ''' <remarks></remarks>
    Friend Function GetXMLNodeObjectValuePairs(ByVal myXMLPath As String, ByVal pathNode As String, _
                ...
                For Each myAttachmentNode As XmlNode In myXMLNode.ChildNodes
                    ...
                    For Each myChildNode As XmlNode In myAttachmentNode.ChildNodes
                        ...
                        If myChildNode.Name = queryNodeName Then
                            ...
                            If StringExistInName(readNodeText, queryNodeValue) Then
                            ...
                            If myChildNode.Name = corrNodeName Then
                                ...
                                Select Case myAction
                                    ...
                                    Case eXMLEditorAction.DirectoriesDBGather
                                        ...
                                        If Not String.IsNullOrEmpty(corrNodeValueStub) Then corrNodeValueStub = TrimPathSlash(corrNodeValueStub) & "\"
...
#Region "Creating/Inserting/Deleting Nodes"
    ...
    ''' <remarks></remarks>
    Friend Sub CreateNodeByPath(ByVal pathNode As String, ByVal nameNode As String, ByVal valueNode As String, _
    ...
    ''' <remarks></remarks>
    Friend Sub DeleteNodeByPath(ByVal pathNode As String, Optional ByVal pathNodeAttrib As String = "")
        'Create an XmlNamespaceManager for resolving namespaces.
        Dim nsmgr As New XmlNamespaceManager(xmlDoc.NameTable)
        nsmgr.AddNamespace("n", xmlDoc.DocumentElement.NamespaceURI)
        myXMLNode = xmlRoot.SelectSingleNode(pathNode, nsmgr)
    ...
    ''' <param name="typeNode">New node type.</param>
    ''' <remarks></remarks>
    Friend Sub CreateNodeInXMLFile(ByVal myPath As String, ByVal pathNode As String, ByVal nameNode As String, ByVal valueNode As String, _
                            ByVal typeNode As eXMLElementType, ByVal createMethod As eNodeCreate)
        Try
            If InitializeXML(myPath) Then
    ...
    ''' <remarks></remarks>
    Friend Sub DeleteNodeInXMLFile(ByVal myPath As String, ByVal deleteNodePath As String, Optional ByVal deleteNodeAttribute As String = "")
        Try
            If InitializeXML(myPath) Then
...
#Region "Creating Node Objects"
    ...
    ''' <remarks></remarks>
    Friend Sub CreateObjectIncident(ByVal p_pathNode As String,
                                           ByVal p_incidents As ObservableCollection(Of Integer))
    ...
    ''' <remarks></remarks>
    Friend Sub CreateObjectTicket(ByVal p_pathNode As String,
                                         ByVal p_tickets As ObservableCollection(Of Integer))
        Dim objXMLNodeHeader As XmlNode
        Dim objXMLNode As XmlNode
        Dim myNameSpace As String
    ...
    ''' <remarks></remarks>
    Friend Sub CreateObjectLink(ByVal p_pathNode As String,
                                       ByVal p_links As ObservableCollection(Of cMCLink))
        Dim objXMLNodeHeader As XmlNode
        Dim objXMLNode As XmlNode
        Dim myNameSpace As String
    ...
    Friend Sub CreateObjectFileAttachment(ByVal p_pathNode As String,
                                          ByVal p_attachments As ObservableCollection(Of cFileAttachment),
                                          ByVal p_headerTitle As String)
        Dim objXMLNodeHeader As XmlNode
        Dim objXMLNode As XmlNode
        Dim myNameSpace As String
    ...
    ''' <remarks></remarks>
    Friend Sub CreateObjectUpdate(ByVal p_pathNode As String,
                                         ByVal p_updates As ObservableCollection(Of cMCUpdate))
        Dim objXMLNodeHeader As XmlNode
        ...
        Try
            ...
            For Each update As cMCUpdate In p_updates
                ...
                ' Create child ID node and append to header node
                ...
                ' Create child nodes and append to subheader node
    ...
    ''' <remarks></remarks>
    Friend Sub CreateObjectResult(ByVal p_pathNode As String,
                                         ByVal p_results As ObservableCollection(Of cMCResult))
        Dim objXMLNodeHeader As XmlNode
        Dim objXMLNodeSubHeader As XmlNode
    ...
    ''' <remarks></remarks>
    Friend Sub CreateObjectResultPostProcessed(ByVal p_pathNode As String,
                                                      ByVal p_resultsPostProcessed As ObservableCollection(Of cMCResultPostProcessed),
                                                      ByVal p_results As ObservableCollection(Of cMCResultPostProcessed))
...
#Region "Mirror XML Functions Into Memory"
    ...
    ''' <remarks></remarks>
    Friend Sub MirrorAllEditorXMLS(Optional ByVal myXMLPathList As List(Of String) = Nothing,
    ...
    ''' <remarks></remarks>
    Friend Function mirrorXMLElementsAll(ByVal p_xmlFilePath As String) As cXMLObject
    ...
    ''' <remarks></remarks>
    Friend Sub Mirror_ChildNodes(ByVal myPath As String, ByRef myChildLevel As Integer, ByVal myChildNodes As XmlNodeList, _
        ...
        If (xmlRoot.Attributes IsNot Nothing And Not myChildLevel > 1) Then
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function IsMatchingHeaderWithoutChildren(ByVal p_xmlNode As cXMLNode,
    ...
    ''' <remarks></remarks>
    Friend Sub CreateMirrorNode(ByVal p_nodePath As String,
    ...
    ''' <remarks></remarks>
    Friend Sub UpdateXMLFile(ByVal myPath As String, ByRef myXMLCollection As ObservableCollection(Of cXMLNode))
        Dim ChildLevel As Integer = 1
    ...
    Friend Sub UpdateXMLFileNodes(ByRef myChildLevel As Integer, ByVal myChildNodes As XmlNodeList, ByRef myCounter As Integer, ByRef myXMLCollection As ObservableCollection(Of cXMLNode), Optional ByVal myCounterTemp As Integer = -2)
