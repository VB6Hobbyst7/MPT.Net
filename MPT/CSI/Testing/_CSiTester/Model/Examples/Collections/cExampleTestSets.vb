...
''' </summary>
''' <remarks></remarks>
Public Class cExampleTestSets
    'TODO: Create based on other collection classes.
    'TODO: Look for methods to bring in, likely from cCSiTester and cSettings
...
#Region "Properties - List"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Default Friend Overloads ReadOnly Property item(ByVal p_index As Integer) As cExampleTestSet
        Get
...
#Region "Initialization"
    ...
    Friend Function Clone() As Object Implements System.ICloneable.Clone
        Dim myClone As New cExampleTestSets

        With myClone
            For Each item As cExampleTestSet In InnerList
                .Add(item)
            Next
        End With
    ...
    Public Overrides Function Equals(ByVal p_object As Object) As Boolean
        If Not (TypeOf p_object Is cExampleTestSets) Then Return False
        Dim isMatch As Boolean = False
        Dim comparedObject As cExampleTestSets = TryCast(p_object, cExampleTestSets)

        'Check for any differences
        If comparedObject Is Nothing Then Return False
        For Each updateOuter As cExampleTestSet In comparedObject
            isMatch = False
            For Each updateInner As cExampleTestSet In InnerList
...
#Region "Methods: Friend - List"
    ...
    ''' <remarks></remarks>
    Friend Overloads Sub Add(ByVal p_items As ICollection(Of cExampleTestSet))
        If p_items Is Nothing Then Exit Sub

        For Each item As cExampleTestSet In p_items
            Add(item)
        Next
    End Sub
    ''' <summary>
    ...
    ''' <remarks></remarks>
    Friend Overloads Function Add(ByVal p_object As cExampleTestSet) As Boolean
        Try
...
#Region "Methods: Private - List"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Overloads Function GetItem(ByVal p_index As Integer) As cExampleTestSet
        Try
            If p_index < 0 Then Throw New ArgumentException("Index {1} cannot be a negative number.", p_index.ToString)
            If p_index >= InnerList.Count Then Throw New ArgumentException("Index is greater than the size of the collection: {1} ", "Index: " & p_index.ToString & " Collection Count: " & InnerList.Count.ToString)

            Return CType(InnerList(p_index), cExampleTestSet)
        Catch argExc As ArgumentException
            RaiseEvent Log(New LoggerEventArgs(argExc))
            Return Nothing
        End Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function MCModelUnique(ByVal p_mcModel As cExampleTestSet) As Boolean
        'Dim mcModelIDs As New ObservableCollection(Of String)

        'For Each mcModel As cExampleTestSet In InnerList
